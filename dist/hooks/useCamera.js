...
var resList = [[640, 480, '480p - 4:3'], [1280, 720, '720p - 16:9'], [1920, 1080, '1080p - 16:9']];
...
var getConstraints = function getConstraints(_ref) {
	...
	var filtredRes = resList.filter(function (res) {
		return res[0] >= width.min && res[0] <= width.max && res[1] >= height.min && res[1] <= height.max;
	});
...
var cameraReducer = function cameraReducer(_ref2, _ref3) {
	...
	switch (type) {
		...
		case 'SELECT':
			return { cameraList: [].concat(_toConsumableArray(cameraList)), selectedCameraId: id };
		default:
			return { cameraList: [].concat(_toConsumableArray(cameraList)), selectedCameraId: selectedCameraId };
	}
};
...
var streamReducer = function streamReducer(_ref4, _ref5) {
	...
	switch (type) {
		...
		case 'SET_STREAM_CONS':
			stream.getVideoTracks()[0].applyConstraints(Object.assign({}, newSelectedConstraint));
			return { stream: stream, permitted: permitted, constraints: constraints, selectedConstraint: newSelectedConstraint };
		case 'RESET_STREAM':
			return { stream: null, permitted: false, constraints: [], selectedConstraint: null };
		default:
			return { stream: stream, permitted: permitted, constraints: constraints, selectedConstraint: selectedConstraint };
	}
};

var useCamera = function useCamera() {
	var _useReducer = (0, _react.useReducer)(cameraReducer, { cameraList: [], selectedCameraId: null }),
	...
	var _useReducer3 = (0, _react.useReducer)(streamReducer, { stream: null, permitted: false, constraints: [], selectedConstraint: null }),
	...
	var changeConstraint = (0, _react.useCallback)(function (newSelectedConstraint) {
		dispatchStream({ type: 'SET_STREAM_CONS', newSelectedConstraint: newSelectedConstraint });
	});
							...
							case 4:
								...
								list.forEach(function (dev) {
									if (dev.kind === 'videoinput') {
										var camObject = { label: dev.label, deviceId: dev.deviceId };
	...
	(0, _react.useEffect)(function () {
		reset();
		var SelectedCamera = !!cameraList.length && selectedCameraId ? selectedCameraId : undefined;
					...
					while (1) {
						switch (_context2.prev = _context2.next) {
							...
							case 4:
								...
								_constraints = !!_stream.getTracks()[0].getCapabilities ? getConstraints(_stream.getTracks()[0].getCapabilities()) : [];
								defaultConstraints = !!_stream.getTracks()[0].getSettings ? _stream.getTracks()[0].getSettings() : null;

								dispatchStream({ type: 'SET_STREAM', newstream: _stream, newConstraints: _constraints, newSelectedConstraint: defaultConstraints });
								_context2.next = 13;
								break;
							...
							case 15:
								return _context2.abrupt('return', setError(new Error('No camera device was found!')));
	...
	return [stream, error, changeCamera, changeConstraint, cameraList, constraints, selectedCameraId, selectedConstraint];
};
