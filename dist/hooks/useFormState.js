  ...
  switch (type) {
    case SET_VALUE:
      var newState = _extends({}, state, {
        formState: _extends({}, state.formState, payload)
      });
  ...
  var setForm = (0, _react.useCallback)(function (node) {
    if (node && node.tagName === "FORM") {
      ...
      node.onchange = function (e) {
        dispatch({
          type: SET_VALUE,
          payload: _defineProperty({}, e.target.name, e.target.type !== "checkbox" ? e.target.value : e.target.checked)
        });
      };
        ...
        for (var _iterator = node.children[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          ...
          if (child.tagName === "INPUT") {
            child.oninvalid = function (e) {
              ...
              dispatch({
                type: SET_ERROR,
                payload: _defineProperty({}, e.target.name, validity)
              });
            };
          }
        }
